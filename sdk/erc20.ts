...
export class ERC20 extends BaseContract {
  ...
  public approve(spender: string, amount?: string): TransactionMethods<ContractMethodReturnType<ERC20Contract, 'approve'>> {
    if (!amount) {
  ...
  public transferFrom(from: string, to: string, amount: string): TransactionMethods<ContractMethodReturnType<ERC20Contract, 'transferFrom'>> {
  ...
  private async _info(): Promise<TypedTokenInfo> {
    ...
    if (!this.chain.isZeroAddress(this.address)) {
      const [name, symbol, decimals] = await Promise.all([this.contract.name(), this.contract.symbol(), this.contract.decimals()]);
      token.name = name;
      token.symbol = symbol;
...
export function getERC20(chain: Chain, address: string): ERC20 {
  console.log('address', address, chain.contractmaps);
